...
##
COPT+=`pkg-config --cflags-only-I --libs-only-L libusb-1.0 libpng`
...
$(TESTDIR)/test_790.prg:       $(TESTDIR)/test_790.c $(TESTDIR)/test_790_asm.s $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map --listing $*.list $(TESTDIR)/test_790_asm.s $< $(MEGA65LIBCLIB)

$(TESTDIR)/test_897.prg:       $(TESTDIR)/test_897.c $(TESTDIR)/test_897_asm.s $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map --listing $*.list $(TESTDIR)/test_897_asm.s $< $(MEGA65LIBCLIB)
...
$(TESTDIR)/production_test.prg:       $(TESTDIR)/production_test.c $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map --listing $*.list $< $(MEGA65LIBCLIB)
...
$(EXAMPLEDIR)/bmpview.prg:       $(EXAMPLEDIR)/bmpview.c $(CC65) $(MEGA65LIBCLIB)
	$(CL65)  $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map $< $(MEGA65LIBCLIB)

$(EXAMPLEDIR)/modplay.prg:       $(EXAMPLEDIR)/modplay.c $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map $< $(MEGA65LIBCLIB)

$(EXAMPLEDIR)/raycaster.prg:       $(EXAMPLEDIR)/raycaster.c $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map $< $(MEGA65LIBCLIB)
...
$(B65DIR)/wirekrill.prg:       $(EXAMPLEDIR)/wirekrill.c $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map $< $(MEGA65LIBCLIB)
...
$(UTILDIR)/gmod2-tools.prg:       $(UTILDIR)/gmod2-tools.c $(CC65) $(MEGA65LIBCLIB)
	$(CL65) $(MEGA65LIBCINC) -O -o $*.prg --mapfile $*.map $< $(MEGA65LIBCLIB)
...
$(BINDIR)/shresls:	$(TOOLDIR)/shresls.c Makefile
...
$(BINDIR)/mega65_ftp.static: $(MEGA65FTP_SRC) $(MEGA65FTP_HDR) $(TOOLDIR)/version.c include/*.h Makefile ncurses/lib/libncurses.a readline/libreadline.a readline/libhistory.a
	$(CC) $(COPT) -Iinclude $(LIBUSBINC) -o $(BINDIR)/mega65_ftp.static $(MEGA65FTP_SRC) $(TOOLDIR)/version.c ncurses/lib/libncurses.a readline/libreadline.a readline/libhistory.a -ltermcap -DINCLUDE_BIT2MCS
