...
void main(void)
{
  ...
  POKE(COLOR+1,0x0f); // First Rowmask: 00001111, i.e. top (bottom?) 4 character pixel lines should be masked (not displayed)
  POKE(COLOR+2,0x00);
  POKE(COLOR+3,0x00);
  ...
  //POKE(SCREEN+2,98); // ** ONLY USED FOR TESTING THE TEST! ** Output the character to mask (Half Inverted space to simulate passing the test)
  POKE(SCREEN+3,0x00);
  ...
  //POKE(SCREEN+2,98+128); // ** ONLY USED FOR TESTING THE TEST! ** Output the character to mask (Half Inverted space to simulate passing the test)

  // run our 2nd test with rowmask of $0f
  POKE(COLOR+1,0xf0); // First Rowmask: 11110000, i.e. top (bottom?) 4 character pixel lines should be masked (not displayed)
  testpixels();
