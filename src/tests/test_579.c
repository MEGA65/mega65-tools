...
void main(void)
{
  ...
  print_text(0, 0, 1, "Issue #579 - Tiles Sprite not Covering  Right-Hand Most Pixel Column");
  ...
  // are stoping being displayed 1 pixel clock tick early, which causes the
  // pixel colour to change early, but not early enough for the pixel colour
  // read-back to pick it up.
  POKE(0xD020, 0);

  // Check a sprite pixel
