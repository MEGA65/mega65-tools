...
void main(void)
{
  ...
  print_text(0, 0, 1, "Issue #334 - Colour $FF not working in   ");
  print_text(0, 1, 1, "full colour mode. Should show 8-bit value");
  print_text(0, 2, 1, "from colour RAM.");
  print_text(0, 3, 2, "Last square should cycle red to blue, not");
  print_text(0, 4, 2, "through standard C64 colours");

  y = 5;
  x = 0;
  for (i = 0; i < 256; i++) {
    char msg[80];
    snprintf(msg, 80, "%02X", i);
    print_text(x, y, 12, msg);
    lfill(0x40000L + y * 64 + (x + 2) * (64 * 25L), i, 64);
    x += 3;
    if (x > 37) {
      x = 0;
      y++;
  ...
  // Set upper 240 palette entries to something interesting
  // red to blue transition (colour blindness friendly)
  for (i = 16; i != 256; i++) {
    POKE(0xD100 + i, nyblswap(255 - i));
    POKE(0xD200 + i, 0);
    POKE(0xD300 + i, nyblswap(i));
  }

  // Get screen position of colour $FF swatch
  x--;

  // Cycle the colour in that last square through the colour transition
  i = 16;
  while (1) {
    i++;
    if (i > 0xff)
      i = 0x10;
    lpoke(0xff80000L + x * 2 + y * 80 + 1, i);
  }
}
