...
void main(void)
{
  ...
  for(i=0;i<38;i++)
    {
      ...
      while(y--) printf("%c",0x11);
      while(x--) printf("%c",0x1d);
      lcopy(0x40000+i*32+3,filename,16);
      filename[16]=0;
      if (i>file_count) filename[0]=0;
      printf("%c. %s",
	     (i<26)?0x41+i:0x30+(i-26),
	     filename);
    }
  ...
  while(1) {
    ...
    i=PEEK(0xD610);
    POKE(0x0402,i);
    if (i>=0x61&&i<=0x76) i=i-0x61;
    else if (i>=0x41&&i<=0x56) i=i-0x41;
    else if (i>='0'&&i<='9') i-=0x30+26;
    else {
      i=-1;
    }
    POKE(0xD610,0);
    POKE(0x400,i); POKE(0x401,file_count);
    if (i<file_count) {
      lcopy(0x40000+i*32+3,filename,16);
      filename[16]=0;
      printf("%cLoading %s...\n",0x93,filename);
