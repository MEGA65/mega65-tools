  ...
  *(unsigned char *)2040 = 0x3c0 / 0x40;
  ...
  while (1) {
    ...
    if (!(a1 & 0x40))
      joy_y++;
    if (!(a1 & 0x8))
      POKE(0xd027U, (PEEK(0xD027U) + 1) & 0xf);
    POKE(0xD000U, joy_x);
    POKE(0xD001U, joy_y);
...
void clip_date(void)
{
  if (tm.tm_mday >= 30)
    tm.tm_mday = 0;
  if ((tm.tm_mon == 2) && (tm.tm_mday > 28))
    tm.tm_mday = 0;
  if (((tm.tm_mon == 4) || (tm.tm_mon == 6) || (tm.tm_mon == 6) || (tm.tm_mon == 9) || (tm.tm_mon == 11))
      && (tm.tm_mday > 29))
    tm.tm_mday = 0;
}
