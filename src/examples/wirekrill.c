...
unsigned char rx_ref[79]={
  0x80, 0xAB,
  0x54, 0x45, 0x53, 0x54, 0x20, 0x4D, 0x45, 0x53, 0x53, 0x41, 0x47, 0x45, 0x20, 0x31, 0x32, 0x33, 
  0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x20, 0x54, 0x48, 0x45, 0x20, 0x51, 0x55, 0x49, 0x43, 
  0x4B, 0x20, 0x42, 0x52, 0x4F, 0x57, 0x4E, 0x20, 0x46, 0x4F, 0x58, 0x20, 0x4A, 0x55, 0x4D, 0x50, 
  0x45, 0x44, 0x20, 0x4F, 0x56, 0x45, 0x52, 0x20, 0x54, 0x48, 0x45, 0x20, 0x4C, 0x41, 0x5A, 0x59, 
  0x20, 0x4F, 0x4C, 0x44, 0x20, 0x44, 0x4F, 0x47, 0x39, 0x93, 0x13, 0xB9, 0xEA
};
  ...
  while (1) {
    ...
    if (PEEK(0xD6E1)&0x20)   // Packet received
      ...
      snprintf(msg,160,"%02x:%02x:%02x:%02x:%02x:%02x > %02x:%02x:%02x:%02x:%02x:%02x\n  len=%d, rxerr=%c",
	       ...
	       frame_buffer[0]+((frame_buffer[1]&0xf)<<8),
      ...
      for(i=0;i<79;i++)
	if (frame_buffer[0x0e+i]!=rx_ref[i]) break;
      if (i>2&&i<79) {
	printf("%s\n",msg);
	printf("Corrupt RX reference packet:\n");
	for(i=0;i<79;i++) {
	  // Red for errors, white for correct
	  if (frame_buffer[0x0e+i]!=rx_ref[i]) printf("%c",28); else printf("%c",5);
