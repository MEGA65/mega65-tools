...
void usage(void)
{
  ...
  fprintf(stderr, "usage: m65 [-l <serial port>] [-s <230400|2000000|4000000>]  [-b <FPGA bitstream> [-v <vivado.bat>] [[-k "
                  "<hickup file>] [-R romfile] [-U flashmenufile] [-C charromfile]] [-c COLOURRAM.BIN] [-B breakpoint] [-o] "
                  "[-d diskimage.d81] [-j] [-J <XDC,BSDL[,sensitivity list]> [-V <vcd file>]] [[-1] [<-t|-T> <text>] [-f "
                  "FPGA serial ID] [filename]] [-H] [-E|-L] [-Z <flash addr>]\n");
...
int load_file(char *filename, int load_addr, int patchHyppo)
{
  ...
  while (b > 0) {
    ...
    if (serial_speed == 230400)
      do_usleep(10000 + 50 * b * SLOW_FACTOR);
...
int stuff_keybuffer(char *s)
{
  ...
  char cmd[1024];
  snprintf(cmd, 1024, "s%x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\rs%x %02x\r", buffer_addr, s[0], s[1], s[2],
      s[3], s[4], s[5], s[6], s[7], s[8], s[9], buffer_len_addr, (int)strlen(s));
...
int virtual_f011_read(int device, int track, int sector, int side)
{
  ...
  if (((gettime_ms() - last_virtual_time) > 100) || (last_virtual_writep) || (last_virtual_track != track)
      || (last_virtual_sector != sector) || (last_virtual_side != side)) {
    ...
    if (result) {
      ...
      if (b == 512) {
        ...
        if (new_monitor)
          sprintf(cmd, "l%x %x\r", READ_SECTOR_BUFFER_ADDRESS, (READ_SECTOR_BUFFER_ADDRESS + 0x200) & 0xffff);
        else
        ...
        if (serial_speed == 230400)
          ...
          do_usleep(10000 + 6 * b * SLOW_FACTOR);
#ifdef WINDOWS
