...
#define DEBUG_NOTIFY_INVALID_IDENTIFIER(chIdentifier)                                                                       \
  DBG_CODE({                                                                                                                \
    char buf[256];                                                                                                          \
    sprintf(buf,                                                                                                            \
        "DEBUG: The format string or return identifier contained the invalid identifier '%c'.\n"                            \
...
template <class T> class CDhComPtr {
  public:
  CDhComPtr() throw()
      : m_pInterface(NULL)
  {
  ...
  CDhComPtr(const CDhComPtr& original) throw()
      : m_pInterface(original.m_pInterface)
  {
    if (m_pInterface)
      m_pInterface->AddRef();
  }
...
/* ===================================================================== */
template <class T> class CDhStringTemplate {
  public:
  ...
  CDhStringTemplate(const int null) throw(HRESULT)
      : m_strptr(NULL)
  {
    if (null != 0)
      throw(E_POINTER);
...
class CDhInitialize {
  public:
  CDhInitialize(const BOOL bInitCom = TRUE) throw()
      : m_bInitCom(bInitCom)
  {
