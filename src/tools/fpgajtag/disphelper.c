...
HRESULT dhInvokeArray(int invokeType, VARIANT* pvResult, UINT cArgs, IDispatch* pDisp, LPCOLESTR szMember, VARIANT* pArgs)
{
  ...
  hr = pDisp->lpVtbl->Invoke(
      pDisp, dispID, &IID_NULL, LOCALE_USER_DEFAULT, (WORD)invokeType, &dp, pvResult, &excep, &uiArgErr);
...
static HRESULT TraverseSubObjects(IDispatch** ppDisp, LPWSTR* lpszMember, va_list* marker)
{
  ...
  do {
    *szSeperator = L'\0';
    ...
    hr = InternalInvokeV(DISPATCH_METHOD | DISPATCH_PROPERTYGET, VT_DISPATCH, &vtObject, *ppDisp, szTemp, marker);
    ...
    int invokeType, VARTYPE returnType, VARIANT* pvResult, IDispatch* pDisp, LPOLESTR szMember, va_list* marker)
{
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr) && pvResult != NULL && pvResult->vt != returnType && returnType != VT_EMPTY) {
...
static HRESULT CreateArgumentArray(LPWSTR szMember, VARIANT* pArgs, BOOL* pbFreeList, UINT* pcArgs, va_list* marker)
{
  ...
  while (*szMember) {
    if (!bInArguments && (*szMember == L'(' || *szMember == L' ' || *szMember == L'=')) {
      bInArguments = TRUE;
...
HRESULT ConvertVariantTimeToTimeT(DATE date, time_t* pTimeT)
{
  ...
  *pTimeT = (time_t)(((date - VARIANT_TIMET_DAY0) * TIMET_ONE_DAY) + 0.5);
  ...
  if ((ptm = gmtime(pTimeT)) == NULL || !(ptm->tm_isdst = -1) || (*pTimeT = mktime(ptm)) == (time_t)-1)
    return E_FAIL;
  ...
  if (FAILED(hr) && !g_ExceptionOptions.bDisableRecordExceptions) {
    ...
    if (pException->hr == 0) {
      ...
      if (pExcepInfo && hr == DISP_E_EXCEPTION) {
        if (pExcepInfo->pfnDeferredFillIn && !IsBadCodePtr((FARPROC)pExcepInfo->pfnDeferredFillIn))
...
HRESULT dhShowException(PDH_EXCEPTION pException)
{
  ...
  MessageBoxW(g_ExceptionOptions.hwnd, szMessage, g_ExceptionOptions.szAppName, MB_ICONSTOP | MB_SETFOREGROUND);
...
#define DESCRIPTION_LENGTH 255
  ...
  if (!pException->szDescription) {
    ...
    if (pException->szDescription) {
      ...
      case DISP_E_MEMBERNOTFOUND:
        _snwprintf(pException->szDescription, DESCRIPTION_LENGTH, L"Object doesn't support this property or method: '%s'",
            pException->szMember);
        break;

      case DISP_E_TYPEMISMATCH:
        if (pException->szMember[0]) {
          _snwprintf(pException->szDescription, DESCRIPTION_LENGTH, L"Type mismatch: '%s'. Argument Index: %d",
              pException->szMember, pException->iArgError);
          break;
        }
...
#endif
        cch = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, hr,
            MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), szDescription, DESCRIPTION_LENGTH, NULL);
  ...
  if (cchBufferSize) {
    if (!bFixedFont) {
      _snwprintf(szBuffer, cchBufferSize,
          L"Member:\t  %s\r\nFunction:\t  %s\t\t\r\nError In:\t  %s\r\nError:\t  %s\r\nCode:\t  %x\r\nSource:\t  %s",
          pException->szCompleteMember, pException->szInitialFunction, pException->szErrorFunction,
          pException->szDescription, hr, pException->szSource);
    }
    else {
      _snwprintf(szBuffer, cchBufferSize,
          L"Member:   %s\r\nFunction: %s\r\nError In: %s\r\nError:    %s\r\nCode:     %x\r\nSource:   %s",
