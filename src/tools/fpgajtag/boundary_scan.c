...
int parse_xdc(char* xdc)
{
  ...
  while (line[0]) {
    if (line[0] != '#') {
      ...
      while (signal_name && signal_name[0] == '{')
        signal_name++;
...
int parse_bsdl(char* bsdl)
{
  ...
  fgets(line, 1024, f);
  while (line[0]) {
    if (sscanf(line, "attribute BOUNDARY_LENGTH of %s : entity is %d;", part_name, &boundary_bit_count) == 2) {
    ...
    int n = sscanf(line, "%*[ \t]\"%*[ \t]%d (BC_%*d, %[^,], %[^,], %[^,)]", &bit_number, bit_name, bit_type, bit_default);
    if (n == 4) {
...
int xilinx_boundaryscan(char* xdc, char* bsdl, char* sensitivity)
{
  ...
  for (int i = 0; i < boundary_bit_count; i++) {
    ...
    if (sensitivity) {
      if (!i)
        printf("Applying sensitivity list '%s'\n", sensitivity);
      if
#ifdef WINDOWS
      ...
      for (int i = 0; i < boundary_bit_count; i++) {
        ...
        if (bbit_show[i] | bbit_vcdchar[i]) {
          if (first_time || last_value != value) {
            ...
            if ((first_time && ((!sensitivity) || vcd)) || (!bbit_ignore[i])) {
              ...
              if (vcd) {
                ...
                printf("bit#%d : %s (pin %s, signal %s) = %x\n", i, boundary_bit_fullname[i], boundary_bit_pin[i],
                    bbit_names[i], value);
            }
          }
        }
