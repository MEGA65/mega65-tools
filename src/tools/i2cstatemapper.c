...
// so we have to schedule multiple jobs.
  ...
  { 0x72, 0x04, 0x05, 0x04, "IO Expander #0 regs 4-5" }, { 0x72, 0x06, 0x07, 0x06, "IO Expander #0 regs 6-7" },

  // Second IO expander
  { 0x74, 0x00, 0x01, 0x08, "IO Expander #1 regs 0-1" }, { 0x74, 0x02, 0x03, 0x0a, "IO Expander #1 regs 2-3" },
  { 0x74, 0x04, 0x05, 0x0c, "IO Expander #1 regs 4-5" }, { 0x74, 0x06, 0x07, 0x0e, "IO Expander #1 regs 6-7" },

  // Third IO expander
  { 0x76, 0x00, 0x01, 0x10, "IO Expander #2 regs 0-1" }, { 0x76, 0x02, 0x03, 0x12, "IO Expander #2 regs 2-3" },
  { 0x76, 0x04, 0x05, 0x14, "IO Expander #2 regs 4-5" }, { 0x76, 0x06, 0x07, 0x16, "IO Expander #2 regs 6-7" },
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; e[i].addr > -1; i++) {
    printf("when %d =>\n"
           ...
           "  i2c1_wdata <= x\"%02X\";\n"
           "  i2c1_rw <= '0';\n",
        busy_count, e[i].desc, (e[i].addr & 0x80) ? '1' : '0', (e[i].addr & 0x40) ? '1' : '0',
        (e[i].addr & 0x20) ? '1' : '0', (e[i].addr & 0x10) ? '1' : '0', (e[i].addr & 0x8) ? '1' : '0',
        (e[i].addr & 0x4) ? '1' : '0', (e[i].addr & 0x2) ? '1' : '0', e[i].addr, e[i].reg_low);
    busy_count++;
    int first = busy_count;
    printf(" when");
    for (int j = e[i].reg_low; j <= e[i].reg_high + 1; j++)
      printf(" %d %s", busy_count++, (j <= e[i].reg_high) ? "|" : "");
    printf("=>\n"
           ...
           "  end if;\n",
        e[i].reg_high - e[i].reg_low + 1, first, first, e[i].reg_offset);
  }
}
