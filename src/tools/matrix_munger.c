...
int main(void)
{
  ...
  for (int key = 0; key < 72; key++) {
    ...
    if (!strcmp("VERT", r1))
      ascii = 0x91;
    if (!strcmp("RUN", r1))
      ascii = 0xa3; // slightly random assignment
    if (!strcmp("TAB", r1))
      ascii = 0x0f;
  ...
  for (int key = 0; key < 72; key++) {
    ...
    if (!strcmp("RUN", r1))
      ascii = 0xa3; // slightly random assignment
    if (!strcmp("TAB", r1))
      ascii = 0xef; // C=+TAB = Matrix Mode trap
  ...
  printf("\n");
  if (reverse_matrix[0x0d])
    printf("  when '\r' => return %d;\n", reverse_matrix[0x0d]);
  for (int i = 32; i < 0x7d; i++) {
    if (reverse_matrix[i])
      printf("  when '%c' => return %d;\n", i, reverse_matrix[i]);
  }
